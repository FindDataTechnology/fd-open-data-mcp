...
def generate(input_path: str) -> CatalogManifest:
    """Generate catalog from fd-edgar seeds/*.py files.
    ...
    manifest = CatalogManifest(
        name="fd-edgar",
        label="SEC EDGAR filings (edgartools)",
    ...
    for cmd, info in all_commands.items():
        ...
        function_spec = {
            "command": cmd,
            "category": info.get("category", "sec_filings"),
            "description": info.get("description", f"{cmd} - SEC EDGAR data"),
            "frequency": "on_demand",
            "parameters": params,
            "columns": columns,
        }
