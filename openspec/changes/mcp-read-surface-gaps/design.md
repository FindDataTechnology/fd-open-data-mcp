...
- Q1: Why does live dispatch fail for uncovered concepts despite working egress? Candidates: entity identifier resolution per source (worldbank wants 3-letter codes), `_build_params` indicator mapping, or `real_sources` failover ordering. Diagnose one concrete case (`gdp`/`AU` via wbgapi) during implementation of task 2 — the fix may be an adapter bug worth its own change; this change only records the finding.
