"""
Universal migration script for entity sync mechanism.
...
# Database connection - prioritize command-line argument, then environment variable
if len(sys.argv) > 1:
...
def migrate():
    ...
    try:
        # Task 4.1: Create entity_sources table
        print("\n[1/6] Creating entity_sources table...")

        if adapter.__class__.__name__ == "PostgreSQLAdapter":
            # PostgreSQL DDL
        ...
        # Task 4.2: Create entity_sync_logs table
        ...
        # Task 4.3: Create entity_sync_schedules table
        ...
        if adapter.__class__.__name__ == "PostgreSQLAdapter":
            ...
            # SQLite doesn't support partial indexes, create regular index
            session.execute(text("""
                CREATE INDEX IF NOT EXISTS idx_entity_sync_schedules_next_run
                ON entity_sync_schedules(next_run_at)
            """))
        ...
        # Task 4.5: Seed initial configuration for 10 entity types
        ...
        for source in initial_sources:
            # Serialize metadata_columns to JSON string for SQLite
        ...
        print(f"\nDatabase type: {adapter.__class__.__name__}")
        print(f"Batch size: {adapter.batch_size}")
        print("\nNext steps:")
        print("  1. Review configurations: SELECT * FROM entity_sources;")
        print("  2. Run sync engine: python -m fd_open_data_mcp.sync.engine")
